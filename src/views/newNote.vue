<template>
  <div class="new-note">
    <h1 class="text-2xl font-bold mb-4">✏️ 새 노트 작성</h1>
    
    <!-- @submit : 폼의 submit 이벤트를 감지-->
    <form @submit.prevent="saveNote" class="space-y-4"> <!-- class="space-y-4": Tailwind CSS 클래스, 폼 내부 요소들 사이에 상하 여백을 4단위(1rem, or 16px) 설정 --> 
      <div class="form-group">
        <label for="title" class="input-text">제목</label> <!--id="title"인 입력필드와 연결-->
        <!--사용자가 텍스트를 입력할 수 있는 필드-->
        <!--
        v-model="title" : 양방향 데이터 바인딩, 입력값이 Vue 컴포넌트의 title변수와 동기화
        placeholder : 필드가 비어있을 때 표시되는 안내 텍스트
        required : 이 필드가 필수 임을 나타내는 속성
        -->
        <input 
          type="text" 
          id="title"
      ...
      <!--
      rows="10" : 10줄 높이의 텍스트 영역 생성
      white-space: pre-wrap : 모든 공백과 줄바꿈을 보존, 자동 줄바꿈도 허용
      -->
      <div class="form-group">
      ...
      <!--이미지 엎로드
      ref="fileInput" : ref로 DOM 요소에 직접 접근할 수 있게 설정 
      @change="handleFileChange" : 파일 선택 시 handleFileChange 함수 호출
...
const saveNote = async() => {
  if (title.value.trim() && content.value.trim()) {
    ...
    store.dispatch('addNote', noteDate);
